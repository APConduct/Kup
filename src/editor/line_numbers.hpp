...
namespace kup {
    ...
    public:
        LineNumberView(std::shared_ptr<Buffer> buf, const Font& font, float font_size, float spacing)
        ...
        void paint(plastic::Context* cx) const override {
            ...
            const auto start_line = static_cast<size_t>(scroll_y / line_height);
            ...
            const Color text_color = get_style().text_color_normal.value_or(GRAY).rl();
